...
class DeepWalk(Estimator):
    ...
    def fit(self, graph):
        ...
        walker.do_walks(graph) # @ANN:utils.random_walk(node):No output, walker.walks is updated

        # @ANN:gensim.models.word2vec(walker.walks, use hierarchical softmax = {0/1},
        # learning rate, epochs,
        # dimensions, int -> Dimensionality of embedding. Default is 128,
        # window_size -> Matrix power order. Default is 5,
        # min_count, int -> Minimal count of node occurences. Default is 1,
        # workers, int -> Number of threads. Default is 4
